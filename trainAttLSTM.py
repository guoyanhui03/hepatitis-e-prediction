...
from AttLSTMModel import AttLSTMModel
...
def train(args):
    min_mape = 1000
    # 创建模型
    model = AttLSTMModel(args.input_size, args.hidden_size, args.num_layers,args.output_size)
    model.to(args.device)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=args.lr)

    # 获取训练集和测试机的数据加载器
    max_num, min_num, train_loader, test_loader = getFabingShuData(args.dataFile, args.sequence_length,
                                                              args.batch_size)
